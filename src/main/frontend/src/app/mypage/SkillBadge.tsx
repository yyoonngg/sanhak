...
const SkillBadge = ({
 skill
  ...
  return (
    <div className={`w-24 h-24 flex flex-col items-center justify-between bg-white p-2 m-4 bg-gray-200 rounded-3xl ${bg_shadow}`}>
      <div className='w-16 h-16'>
        <img className='w-full h-full object-contain' src={image_src} alt={skill.name}/>
      </div>
      <div className='h-4 font-bold text-sm'>{skill.name}</div>
    </div>
  )
};
